...
void sha256(const unsigned char* data, size_t len, unsigned char* out) {
    ...
    for (size_t idx = 0; idx < chunk_len; idx++) {
        ...
        for (int i = 0; i < 64; i++) {
            ...
            uint32_t ch = (e & f) ^ (~e & g);
            uint32_t temp1 = h_ + s_1 + ch + k[i] + w[i];
    ...
    copy_uint32(out, h[0]);
    copy_uint32(out + 4, h[1]);
    copy_uint32(out + 8, h[2]);
    copy_uint32(out + 12, h[3]);
    copy_uint32(out + 16, h[4]);
    copy_uint32(out + 20, h[5]);
    copy_uint32(out + 24, h[6]);
    copy_uint32(out + 28, h[7]);
    for (int i = 0; i < 8; i++) {
        out[4 * i] = h[i] >> 24;
        out[4 * i + 1] = h[i] >> 16;
        out[4 * i + 2] = h[i] >> 8;
    ...
    char out_temp[99] = { 0 };
    for (int i = 0; i < 32; i++) {
        sprintf(out_temp + 2 * i, "%02x", out[i]);
    }
    strcpy((char*)out, out_temp);
}